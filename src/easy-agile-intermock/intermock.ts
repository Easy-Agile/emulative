...
import { FUNCTION_MOCK } from "../constants";
import { FIXED_ARRAY_COUNT, DEFAULT_ARRAY_RANGE } from "./lib/constants";
import {
  defaultTypeToMock,
...
function generatePrimitive(
  ...
  if (mockType) {
    ...
    if (!defaultTypeToMock[syntaxType]) {
      return `This is a mock stub due to a currently unsupported type`;
      ...
      if (record.kind !== record.aliasedTo) {
        ...
        if (isPrimitiveType) {
          ...
          if (record && record.node) {
            ...
            ((record.node as ts.TypeAliasDeclaration).type as any).types = // TODO: Type here was ts.UnionOrIntersectionTypeNode,
              updatedArr as unknown as ts.NodeArray<ts.TypeNode>;
